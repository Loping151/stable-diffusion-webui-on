...
def tokendealer(self, p):
    ...
    if forge:
        ...
        elif hasattr(p.sd_model, "text_processing_engine"):
            tokenizer = p.sd_model.text_processing_engine.tokenize_line
        else:
            # Architectures without a CLIP text_processing_engine (e.g. Anima's Qwen3 text
            # encoder) can't use Regional Prompter's token accounting; skip it cleanly instead
            # of crashing on the missing attribute.
            print("Regional Prompter: model has no CLIP tokenizer (e.g. Anima); skipping RP.")
            return True
    else:
